...
local function loadFile(path, window)
    ...
    gui.log:info('Loading file: %s', path)
    window:hide()
    local f = fs.open(path, 'r')
    if not f then
        gui.log:error('Unable to open file "%s" for read', path)
        return
    end
    local file = textutils.unserialiseJSON(f.readAll())
    f.close()
    if not file then
        gui.log:error('File "%s" corrupted', path)
        return
    end
    gui.loadTemplate(file.template)
    gui.template.load(file)
end
...
local function saveFile(path, window)
    ...
    gui.log:info('Saving file: %s', path)
    local f = fs.open(path, 'w')
    if not f then
        gui.log:error('Unable to open file "%s" for write', path)
        return
    end
    f.write(textutils.serialiseJSON(gui.template.getFields()))
    f.close()
end
...
local function exportFile(path, window)
    ...
    gui.log:info('Exporting file: %s', path)
    local f = fs.open(path, 'w')
    if not f then
        gui.log:error('Unable to open file "%s" for write', path)
        return
    end
    f.write(gui.template.export())
    f.close()
end
...
gui.mo_file = pos.gui.MenuOption(6, 'File', {'Load','Save','Export','Print'}, 8, function(index, option)
    ...
    if option == 'Load' then
        gui.fileSel:updateFiles(gui.fileSel.root)
    ...
    elseif option == 'Print' then
        ...
        for pageN, page in pairs(pages) do
            printer.newPage()
            printer.setPageTitle(gui.template.name .. ' ' .. pageN .. '/' .. #pages)
...
function gui.loadTemplate(template)
    ...
    if not good then
        gui.log:warn('Tried to open unknown template: "%s"', tostring(template))
        return
    end
    local f, e = loadfile('types/' .. template .. '.lua')
    if (not f) or e then
        gui.log:error('Unable to open type: %s', template)
        return
    end
    gui.template = f(gui.body)
end
